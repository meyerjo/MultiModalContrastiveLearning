...
class Scene_Net_Dataset(VisionDataset):
    ...
    def __init__(self, root, train=True,
        ...
        super(Scene_Net_Dataset, self).__init__(root)
        ...
        # assume we only have two folders for the two modalities
        assert(len(folders_in_root) == 2)
        modalities = folders_in_root
        class_dirs_in_modalities = [sorted(list(set(os.listdir(_p)))) for _p in modalities]
        assert class_dirs_in_modalities[0] == class_dirs_in_modalities[1], 'set of class directories is not the same'

        # This class is assumed to be
        class_dirs = class_dirs_in_modalities[0]
        print('Class dirs found: {}'.format(len(class_dirs)))

        # make sure that the found classes correspond to the valid
        assert sorted(list(set(self.valid_classes))) == sorted(list(set(class_dirs))), 'class_dirs do not correspond to valid directories'

        # things that have to be filled
        # entry = {'data': [...], 'modality': [...]}
        self.data = []
        self.targets = []
        self.classes = sorted(list(set(class_dirs)))

        if download:
            raise NotImplementedError(
                'Download Scenenet')

        file_regex = re.compile('^(.*)_(depth|photo)\.(png|jpg)$')
        ...
        for _class_dir in class_dirs:
            ...
            for _mod in modalities:
                _full_path = os.path.join(root, _mod, _class_dir)
                _files_given_modality = os.listdir(_full_path)
                _files_given_modality = list(map(lambda x: file_regex.search(x).group(1), _files_given_modality))
                files_in_modalities.append(_files_given_modality)
            ...
            for file in set_modality_files:
                # generate the paths
                _rgb_path = os.path.join(root, 'images', _class_dir, file + '_photo.jpg')
                _depth_path = os.path.join(root, 'depth', _class_dir, file + '_depth.png')
